...
export class ContextRequestInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        ctx.devicePlatform = request.headers[this.configService.headerKey.appPlatform];
        ctx.deviceVersion = request.headers[this.configService.headerKey.appVersion];
        ctx.timezone = request.headers[this.configService.headerKey.timezone] ?? this.configService.app.timezone;
